...
const Gasbuyments = () => {
  const {setgasmodles , gasmodels} = useContext(Modelcontex)
  const componentRef = useRef();
  const handlePrint = useReactToPrint({
    content: () => componentRef.current,
  });
  return (
  //   <div className=' p-4 darks h-28 w-60 mr-4 rounded'>
  //   <h1>Total Gas</h1>
  //   <div class="w-full bg-gray-200 rounded-full dark:bg-gray-700 mt-1">
  //     <div class="btn2 text-sm h-1 font-medium text-blue-100 text-center leading-none rounded-full" 
  //     style={{width:"90%"}}> 
  //     </div>
  //    </div>
  //   <button className='w-full p-1 btn2 rounded-sm mt-1' 
  //   onClick={handlePrint}>Print</button>
  // </div>
    <>
    
     <div className='flex  space-x-2 px-3 darks py-7  items-center rounded'>
            <div><FaGasPump className='text-6xl text-[#4aac93]'/></div>
            <div className='w-full' onClick={() => setgasmodles(true)}>
              {/* <Link to={'/layout/Admin/Report/Gasprintpage'}> */}
                <h1 className='font-press font-semibold text-[#4aac93]  darks'>Report</h1>
                <p className='text-sm'>Total Gas</p>
