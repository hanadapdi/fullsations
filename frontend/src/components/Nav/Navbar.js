...
import Notivication from '../Notivication/Notivivation'
import Noticicationcontex from '../../Statemagemant/Notivication'
import Mesagecontex from '../../Statemagemant/MessageState'
import Message from '../message/Message'
const Navbar = () => {
    const {slidebar , setSlidebar ,showslides} =useContext(SlidestateManagement)
    const {themewhite , themeBlack , onhendel , enabled , hendalsetting , Setting} = useContext(stateNav)
    const {proshow ,profile} = useContext(prState)
    const {notivications , Notivic} = useContext(Noticicationcontex)
    const {handelmessage , mesageshow} = useContext(Mesagecontex);
    const variants = {
        open: { opacity: 1, x: 0 },
  ...
  return (
    ...
    <div className='flex md:mx-5  mx-2 justify-between w-full h-full  items-center '>
        ...
        <ul className='flex space-x-2 text-xl items-center'>
           ...
           <li onClick={handelmessage} 
            className={'p-2 hidden sm:inline  rounded-full transition ease-in-out '}>
